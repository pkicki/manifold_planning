...
from utils.data import unpack_data_linear_move, unpack_data_kinodynamic


class IiwaPlanner(tf.keras.Model):
    def __init__(self, N, n_pts_fixed_begin, bsp, bsp_t, n_dof=6):
    ...
    def __call__(self, x, mul=1.):
        x, q0, xyz0, xyzk, q_dot_0, q_ddot_0 = self.prepare_data(x)
...
class IiwaPlannerLinearMove(IiwaPlanner):
    def __init__(self, N, n_pts_fixed_begin, bsp, bsp_t, n_dof=6):
        super(IiwaPlannerLinearMove, self).__init__(N, n_pts_fixed_begin, bsp, bsp_t, n_dof)
    ...
    def prepare_data(self, x):
        ...
        return x, q0, xyz0, xyzk, q_dot_0, q_ddot_0



class IiwaPlannerKinodynamic(IiwaPlanner):
    def __init__(self, N, n_pts_fixed_begin, bsp, bsp_t, n_dof=7):
        super(IiwaPlannerKinodynamic, self).__init__(N, n_pts_fixed_begin, bsp, bsp_t, n_dof)
