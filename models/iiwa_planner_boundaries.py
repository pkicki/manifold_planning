...
class IiwaPlannerBoundaries(tf.keras.Model):
    ...
    def __call__(self, x, mul=1.):
        ...
        #q_dot_mul = tf.reduce_max(tf.abs(q_dot_) / Limits.q_dot[np.newaxis, np.newaxis], axis=(-1, -2))
        #q_ddot_mul = tf.reduce_max(tf.abs(q_ddot_) / Limits.q_ddot[np.newaxis, np.newaxis], axis=(-1, -2))

        #exp_t_q_dot = q_dot_mul
        #exp_t_q_ddot = tf.sqrt(q_ddot_mul)
